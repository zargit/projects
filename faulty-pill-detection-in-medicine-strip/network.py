...
class Network():
    ...
    def saveUpdate(self):
        np.save("latest_update",(self.biases,self.weights))
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        tracking progress, but slows things down a lot."""
        ...
        for j in xrange(epochs):
            ...
            print "Saving Result of Epoch {0}.".format(j)
            self.saveUpdate() # saving the updated bias and weights
            if test_data:
                acc = self.evaluate(test_data);
                print "Epoch {0}: {1} / {2}".format(
                    j, acc, n_test)
                print "Accuracy : {0}".format(((acc*1.0)/n_test)*100.0)
            else:
